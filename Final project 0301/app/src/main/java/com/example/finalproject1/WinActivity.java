...
public class WinActivity  extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Boolean player1win = intent.getBooleanExtra("player1win", false);
        Boolean player2win = intent.getBooleanExtra("player2win", false);

        FirebaseDatabase.getInstance().getReference("Users")
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        int gold1 = snapshot.child(player1).child("gold").getValue(int.class);
                        int gold2 = snapshot.child(player2).child("gold").getValue(int.class);
                        if(player1win){
                            FirebaseDatabase.getInstance().getReference("Users")
                                    .child(player1).child("gold").setValue(gold2 + gold1);
                            FirebaseDatabase.getInstance().getReference("Users")
                                    .child(player2).child("gold").setValue(0);
                        }
                        else if(player2win){
                            FirebaseDatabase.getInstance().getReference("Users")
                                    .child(player2).child("gold").setValue(gold2 + gold1);
                            FirebaseDatabase.getInstance().getReference("Users")
                                    .child(player1).child("gold").setValue(0);
                        }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) { //every options menu click do something accordingly
        ...
        switch (item.getItemId()) {
            ...
            case R.id.logOut:
                mAuth.signOut();
                FirebaseDatabase.getInstance().getReference("Users").child(account).child("loggedIn").setValue(false);
                Intent newActiv = new Intent(WinActivity.this, LoginActivity.class);
                mAuth.signOut();
                stopService(serviceIntent);
                newActiv.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                newActiv.putExtra("account", account);
                WinActivity.this.startActivity(newActiv);
                break;
        }
